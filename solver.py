...
def get_captcha(iid, dev, brn, typ, session: requests.Session = None):
    ...
    try:
        ...
        res = session.get("https://rc-verification-sg.tiktokv.com/captcha/get", headers=headers, params=params, timeout=15, verify=False)
...
def verify_captcha(payload_dict, params, session: requests.Session = None):
    ...
    try:
        ...
        headers.update(signs)        
        res = session.post("https://rc-verification-sg.tiktokv.com/captcha/verify", headers=headers, params=params, data=json_data, timeout=15, verify=False)
        if res.status_code != 200:            
            return None
        j = res.json()
        if "edata" in j:
